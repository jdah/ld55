...
STBM__API size_t stbm_heap_outstanding(stbm_heap *heap);
// returns currently outstanding allocated bytes
...
typedef void   STBM_CALLBACK stbm_system_free (void *user_context, void *ptr);
...
// the heap data structure at the same time. It should be non-NULL if
...
#ifdef STB_MALLOC_IMPLEMENTATION
...
#define STBM__SMALL_ALIGNMENT        (4 * sizeof(void*))    // 8 or 16
...
#ifdef STBM__MUTEX
  #define STBM__ACQUIRE(lock)  if (lock) STBM_MUTEX_ACQUIRE(lock); else
  #define STBM__RELEASE(lock)  if (lock) STBM_MUTEX_RELEASE(lock); else
#else
...
static int stbm__is_pow2(size_t num)
{
   return (num & (num-1)) == 0;
}
...
typedef struct stbm__system_allocation
{
   ...
   stbm__uintptr                   size; // used only for medium blocks
...
static void stbm__large_free(stbm_heap *heap, void *ptr)
{
   ...
   heap->system_free(heap->user_context, a->system_allocation);
}
...
#define STBM__TLSF_BITMAP_BIT(x)  ((int) (1u << (31u - (x))))
...
static void stbm__medium_link(stbm_heap *heap, struct stbm__medium_freeblock *block, size_t size)
{
   ...
   if (list)
      ...
      heap->medium.toplevel_bitmap |= (1u << (31u-is.index));
      heap->medium.bitmap[is.index] |= (1u << (31u-is.slot));
   }
...
static void stbm__medium_unlink(stbm_heap *heap, struct stbm__medium_freeblock *block)
{
   ...
   if (heap->medium.lists[index][slot] != block) {
      ...
      if (next)
         ...
         heap->medium.bitmap[index] &= ~(1u << (31u-slot));
         if (heap->medium.bitmap[index] == 0)
            heap->medium.toplevel_bitmap &= ~(1u << (31u-index));
      }
   }
}
...
static void stbm__medium_check_list(stbm_heap *heap, int index, int slot)
{
   ...
   STBM_ASSERT((cur == NULL) == ((heap->medium.bitmap[index] & (1u << (31u-slot)))==0));
   while (cur) {
      stbm__index_slot is;
...
static void stbm__medium_check_all(stbm_heap *heap)
{
   ...
   for (i=0; i < STBM__NUM_INDICES; ++i) {
      if (((heap->medium.toplevel_bitmap & (1u << (31u-i)))==0) != (heap->medium.bitmap[i]==0))
         STBM_FAIL("check");
...
static void stbm__alloc_free_debug(stbm_tc *tc, stbm_heap *safe_heap, void *ptr)
{
   ...
   if (!STBM__DEBUG_TESTMAGIC(da->prefix)) STBM_FAIL("Invalid block header in stbm_free");
...
STBM__API size_t stbm_heap_outstanding(stbm_heap *heap) {
    return heap->cur_outstanding_allocations_bytes;
}
...
STBM__API void stbm_heap_free(stbm_heap *heap)
{
   ...
   while (cur != &heap->system_sentinel) {
      ...
      heap->system_free(heap->user_context, cur);
      cur = next;      
   }
   if (heap->medium.cached_system_alloc)
      heap->system_free(heap->user_context, heap->medium.cached_system_alloc);
}
...
static void stbm__mysystem_free(void *context, void *ptr)
{
   free(ptr);
   ++stbm__num_sys_free;
}
...
static int stbm__ptr_compare(const void **p, const void **q)
{
...
#endif // STB_MALLOC_IMPLEMENTATION
